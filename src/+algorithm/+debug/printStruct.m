function printStruct(data, file)

    function printStruct2(str, data)

        switch (class(data))

            case 'struct'

                cellfun(@(name) printStruct2(strcat(str, '.', name), data.(name)), fieldnames(data));

            case 'cell'

                for pos = 1 : length(data)

                    printStruct2(strcat(str, '{', num2str(pos), '}'), data{pos});
                end
